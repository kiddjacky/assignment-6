...
-(instancetype) initWithURL:(NSURL *) url {
    ...
    if (self) {
        ...
        _folder = [[[_fileManager URLsForDirectory:NSCachesDirectory
                   ...
                   isDirectory:YES];
        BOOL isDir = NO;
        if (![_fileManager fileExistsAtPath:[_folder path] isDirectory:&isDir]) {
...
{
    NSDirectoryEnumerator *dirEnumerator = [self.fileManager enumeratorAtURL:self.folder
                                                  includingPropertiesForKeys:@[NSURLAttributeModificationDateKey] options:NSDirectoryEnumerationSkipsHiddenFiles
                                                                errorHandler:nil];
    NSNumber *fileSize;
    NSDate *fileDate;
